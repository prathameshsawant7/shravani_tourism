...
function getListing(page,order){
    ...
    $("#table_tour_list").load("fetch_ashtavinayak_pickup.php",{"arrangeOrder":arrangeOrder,"cRows":cRows,"page":page,"id":id,"point":point,"added_by":added_by,"updated_by":updated_by}, function(){ //get content from PHP page
...
{
    window.location.href = "add_ashtavinayak_pickup.php?id="+id+"&action=edit";
}
function deletePickup(id)
{
    if(id != ''){
        $.post("ajax_calls.php",{request:'deleteAshtavinayakPickup',id:id},function(data) {
            alertify.alert("Pickup ID: "+id+" - "+trim(data)+" has been deleted successfully.", function(){
