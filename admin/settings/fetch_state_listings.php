...
if(isset($_POST['id_state']) && $_POST['id_state'] != '')
    $filter  .= "AND t.id_state = ".$_POST['id_state']." ";

if(isset($_POST['state']) && $_POST['state'] != '')
    $filter  .= "AND t.state LIKE '%".$_POST['state']."%' ";
...
if(isset($_POST['arrangeOrder'])){
    $OrderBy = "t.id_state ".$_POST['arrangeOrder'];
}
else{
    $OrderBy = "t.id_state DESC";
}

$query = "SELECT COUNT(*) FROM states AS t WHERE ".$filter;
...
$query = "SELECT t.id_state,t.state,t.active "
            . "FROM states AS t "
            . "WHERE ".$filter." "
...
while($state_data = $fetch_data->fetch_assoc()){ //fetch values
    $count++;
    $active = $state_data['active'] ? 'Yes' : 'No';
    $listingHtml .= <<<HEREDOC
    <tr id="tr_{$state_data['id_state']}" style="border:1px solid #000000;">
        <td><center><label>{$state_data['id_state']}</label></center></td>
        <td><center><label>{$state_data['state']}</label></center></td>
        <td><center><label>{$active}</label></center></td>
        <td>
            <center>
                <input type="button" class="small button" value="Edit" onclick="editState({$state_data['id_state']})" style="margin-bottom: -5px;" />
            </center>
        </td>
        <td>
            <center>
                <input type="button" class="small button" value="Delete" onclick="deleteState({$state_data['id_state']})" style="margin-bottom: -5px;"/>
            </center>
        </td>
    </tr>
HEREDOC;
...
echo <<<HEREDOC
    <table border="4">
        <tr>
           <th colspan="5">
                <h3 style="color: #000"><center>State Listings</center></h3>
                <section>
                    ...
                    <span style="float: left;padding-left: 22px;margin-top: -5px;">
                        <a href="add_state.php" class="small button">Add New State</a>
                    </span>
        ...
        <tr style="border:1px solid #000000;">
            <td style="width: 10%;" >
                ...
                <input type="text" id="id_state" class="filter" placeholder="ID" value=""/>
            </td>
            <td>
                <center><label><b>State Name</b></label></center>
                <input type="text" id="state" class="filter" placeholder="State Name" value="State Name"/>
            </td>
            <td>
