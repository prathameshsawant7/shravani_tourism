        ...
        <center>
            ...
            <?php 
                ...
                if(!empty($_GET['action']) && $_GET['action'] == 'edit'){
                    $id         = $_GET['id'];
                    $query      = "SELECT * FROM ashtavinayak_pickup_drop WHERE id=$id and type='drop';";
                    $fetch_data = mysqli_query($con,$query);
                    $drop_data  = $fetch_data->fetch_assoc();
                    $page_action= $_GET['action'];

                    if(!empty($_GET['msg']) && $_GET['msg'] == 'update_success'){
                        echo "Drop ID - ".$_GET['id']. " has been updated successfully.";
                    }
                }else if(!empty($_GET['id'])){
                    echo "Drop ID - ".$_GET['id']. " has been added successfully.";
                } 
            ?>
            </h4>
        </center>
        <form action="insert_tour.php" name="register" method="post">
        <input type="hidden" id="data" name="data" value="ashtavinayak_drop" />
            ...
            <div class="tabs-content vertical" data-tabs-content="example-vert-tabs" style="margin-left: 30%;">
                ...
                <table border="4">
                    <tbody>
                        <tr>
                           <th colspan="2">
                                <h3 style="color: #000"><center>Add/Edit Drop Points</center></h3>
                
                           </th> 
                        </tr>
                        
                        <tr>
                            <th style="border:1px solid #000000;"><label>Drop ID: </label></th>
                            <td> <input type="text" id="id" name="id" value="<?php echo $drop_data['id'];?>" disabled /></td>
                        </tr>
                        <tr>
                            <th style="border:1px solid #000000;"><label>Drop Point: </label></th>
                            <td > <input type="text" id="point" name="point" value="<?php echo $drop_data['point'];?>" /></td>
                        </tr>
