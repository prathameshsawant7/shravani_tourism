...
while($drop_data = $fetch_data->fetch_assoc()){ //fetch values
    $count++;
    #$active = $drop_data['active'] ? 'Yes' : 'No';
    $listingHtml .= <<<HEREDOC
    <tr id="tr_{$drop_data['id']}" style="border:1px solid #000000;">
        <td><center><label>{$drop_data['id']}</label></center></td>
        <td><center><label>{$drop_data['point']}</label></center></td>
        <td><center><label>{$drop_data['added_by']}</label></center></td>
        <td><center><label>{$drop_data['updated_by']}</label></center></td>
        <td>
            <center>
                <input type="button" class="small button" value="Edit" onclick="editDrop({$drop_data['id']})" style="margin-bottom: -5px;" />
            </center>
        </td>
        <td>
            <center>
                <input type="button" class="small button" value="Delete" onclick="deleteDrop({$drop_data['id']})" style="margin-bottom: -5px;"/>
            </center>
        </td>
    </tr>
HEREDOC;
...
echo <<<HEREDOC
    <table border="4">
        <tr>
           <th colspan="6">
                <h3 style="color: #000"><center>Drop Points</center></h3>
                <section>
                    ...
                    <span style="float: left;padding-left: 22px;margin-top: -5px;">
                        <a href="add_ashtavinayak_drop.php" class="small button">Add New Drop</a>
                    </span>
